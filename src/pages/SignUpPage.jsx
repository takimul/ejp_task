...
const SignUpPage = () => {
  ...
  useEffect(() => {
    setIsClient(true); // Set client-side rendering flag to true
  }, []);
  ...
  const onSubmit = async (data) => {
    ...
    // Make sure this API call happens on the client side
    const res = await useAxios.post("/sign-up/api", email);
    console.log(res.data);
    if (res.data.status === 200) {
      setExist(false);
  ...
  if (!isClient) {
    return null;  // Optionally, you can show a loading spinner here while waiting for the client-side to be ready
  }
